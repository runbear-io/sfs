...
| Folder permissions (hub proxy) | done | Added 2026-09-06, after the feature shipped to the hub without a row. `/folders` and `/scope` were answered from the local registry, which carries no rules — so a restricted folder showed no lock, and rule edits 403d against an admin gate the desktop resolver can never satisfy. Now proxied. Check: `TestDesktopFolderRulesProxy` + `desktop.spec.ts` "a folder the hub restricts shows its lock in the app" (verified to fail without the proxy) |
| Viewer reads count toward heat | done | Added 2026-09-06. The sidecar keeps no ReadLedger and answers the viewer routes locally, so a person reading in the app was recorded nowhere while the same file in the web app counted — the dashboard under-reported the reader the app is for. Reported to the project's hub as HUMAN traffic (`Server.ReportRead` hook + `desktop_reads.go`); routing them through the existing report route would have filed a person's browsing as device traffic. Check: `TestDesktopViewerReadReachesTheHub` (mutation-checked) + `TestReadReportCountsAHumanReadAsHuman` |
