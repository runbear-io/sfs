...
| Sign out (hub revocation first) | done | `TestDesktopSessionFlow`. Production finding (2026-08-20 install test): beardrive.ai cloud answers `DELETE /api/auth/token` with 405 — the managed provider doesn't implement revocation, so tokens end only locally there (cloud-repo fix; the sidecar's `revoke_error` reporting works, but the tray discards it — surface it as a notification when the cloud endpoint lands) |
...
| First-run empty state tells the truth about sign-in | open | Found by the 2026-08-20 fresh-install test: signed out with zero projects, the welcome page says "You're signed in, but you're not part of any project yet" — wrong on desktop, where auth.enabled is false but a hub session may not exist. EmptyState should key off `config.desktop && !session.signed_in` (via `/api/desktop/session`) and lead with Sign in. Check: e2e spec on a signed-out harness |
