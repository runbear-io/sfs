...
classDiagram
    ...
    class components {
        ...
        Insights ShareDialog InviteDialog NewProjectDialog
    ...
    note for components "InviteDialog is ShareDialog's two-phase shape moved one step EARLIER: ProjectSettings' owner-only mint no longer POSTs on click — phase 1 names the grant (an org invite also creates an account on an invite-only hub) and picks a TTL, phase 2 renders the minted URL. The `?p=<project-id>` form exists ONLY here: the invite the hub stores and OrgAdmin lists is org-scoped, so the old copy-failure toast pointing at Organization settings handed back a link that had silently lost its project. Expiry sits pre-mint, the opposite of ShareDialog, because CreateInvite always mints a fresh token — there is no already-copied link to keep valid"
