...
| `bdrive sync [folder]` | Run one sync cycle now. `--note <text>` stamps session context (e.g. an agent session id) onto changes — shown in `bdrive log` and hub history; keeps applying to daemon-committed changes until `--note-ttl` (default 30m) expires. A plain `bdrive sync` with no `--note` clears it, so a hand edit is never stamped with the last agent session's note. `--prune` also removes from the hub what `.bdriveignore` now excludes (files stay on disk everywhere). `--hook <label>` is agent-hook plumbing: event JSON on stdin, sync + note, and the turn-start brief (Claude Code hook JSON) on stdout — the gated-link formula, what teammates changed since the last turn, synced files that looked like they hold credentials, the docs this project's own code has outrun (the `bdrive stale` verdict, capped at 3), and a pointer to a synced `AGENTS.md`. The stale half is computed locally from the journal under a wall-clock budget: past it the sentence is dropped, never the turn |
