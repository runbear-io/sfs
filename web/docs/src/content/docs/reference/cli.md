...
| `bdrive sync [folder]` | Run one sync cycle now. Refuses folders this device never `init`ed and folders paused by `bdrive stop`. `--note <text>` stamps session context onto changes; `--note-ttl` (default 30m) bounds it, and a plain `bdrive sync` with no `--note` clears it. `--prune` also removes from the hub what `.bdriveignore` now excludes (files stay on disk everywhere). `--hook <label>` is agent-hook plumbing: it also hands the agent the turn's brief — the files teammates changed since its last turn, the docs this project's own code has outrun (the `bdrive stale` verdict, three worst), and the project's `AGENTS.md` if it syncs. All computed locally, all advisory, and dropped rather than delaying the turn |
